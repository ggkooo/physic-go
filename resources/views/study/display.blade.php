<div class="d-flex flex-column align-items-center mt-4">

    <div class="col-md-9">
        <div class="d-flex justify-content-center mb-4">
            <h5>CONTEÚDOS</h5>
        </div>

        <div class="d-flex flex-column align-items-center mt-4">
            <div class="col-12 col-md-4 mb-4">
                <button class="btn btn-danger w-100 p-3 btn-default btn-raise">ENSINO FUNDAMENTAL</button>
            </div>
            <div class="col-12 col-md-4 mb-4">
                <button class="btn btn-danger w-100 p-3 btn-default btn-raise">QUALIFICAÇÃO - 9° ANO</button>
            </div>
            <div class="col-12 col-md-4 mb-4">
                <button class="btn btn-danger w-100 p-3 btn-default btn-raise">ENSINO MÉDIO</button>
            </div>
        </div>

    </div>

    <div class="row">
        <a href="/home"
            class="btn-exit text-decoration-none text-white w-100 p-1 position-relative d-inline-block overflow-hidden mt-4 mb-3">
            <i class="bi bi-arrow-left icon" style="font-size: 20px; margin-top: 2px;"></i>
            <span class="exit-text" style="font-size: 18px;">Voltar</span>
        </a>
    </div>

</div>

<footer class="text-center fixed mb-3 mt-3">
    <a href="https://www.unijui.edu.br/" target="_blank"><img src="{{ asset('assets/img/unijui_branco.png') }}"
            alt="Unijuí" width="150" class="img-fluid"></a>
</footer>